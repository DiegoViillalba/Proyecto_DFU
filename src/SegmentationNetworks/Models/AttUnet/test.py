...
checkpoint1 = torch.load("output_assets_model/best_model_checkpoint_AttentionUnet.pth", weights_only=True)  ## Nota: el argumento weights_only=True es para evitar el warning que indica que de esta forma se carga con mayor seguridad el modelo. Sin embargo no se están cargando otros datos como el optimizador. En resumen, esto es solo para quitar el warning pues en principio no hay datos maliciosos en la forma en que se guarda el modelo localmente.
model1 = AttentionUnet(in_channels=3, out_channels=1).to(DEVICE)    ## ------------ Aquí al cambiar de modelo -------------.
...
pd.DataFrame(test_metrics, index=[0]).to_csv("output_assets_model/test_metrics_AttentionUnet.csv", index=False)
