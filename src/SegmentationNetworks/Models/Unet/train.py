...
DROPOUT_P = 0.20  # Dropout probability (0.0 to not implement dropout)
...
def main(NUM_EPOCHS=NUM_EPOCHS):
    ...
    # if LOAD_MODEL:  # colab
    #     try:
    #         load_checkpoint(torch.load("/content/DFU_Proyect/SegmentationNetworks/Models/Unet/output_assets_model/best_model_checkpoint.pth",  weights_only=True), model)
    ...
    if SAVE_MODEL:
        ...
        # Save parameters:
        parameters = {'Num Epochs': NUM_EPOCHS, 'Learning Rate': LEARNING_RATE, 'Batch Size': BATCH_SIZE, 'Image Height': IMAGE_HEIGHT, 'Image Width': IMAGE_WIDTH, 'Device': str(DEVICE), 'Num Workers': NUM_WORKERS, 'Pin Memory': PIN_MEMORY, 'Load Model': LOAD_MODEL, 'Save Images': SAVE_IMS, 'Train Image Dir': TRAIN_IMG_DIR, 'Val Image Dir': VAL_IMG_DIR, 'Elapsed Time[s]': round((end_time - start_time)/60, 4), 'Dropout Probability': DROPOUT_P, 'Patience (early_stop)': PATIENCE, 'Optimizer': 'Adam', 'Scheduler': 'ReduceLROnPlateau', 'Loss Function': 'Dice Loss', 'Scaler': 'GradScaler', 'Execution date': datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Model': 'Unet'}
