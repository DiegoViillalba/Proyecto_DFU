...
DROPOUT_P = 0.25  # Dropout probability (0.0 to not implement dropout)
...
def main(NUM_EPOCHS=NUM_EPOCHS):
    ...
    model = UnetPlusPlus(in_channels=3, out_channels=1, dropout=DROPOUT_P).to(DEVICE)
    ...
    if LOAD_MODEL:
        try:
            load_checkpoint(torch.load("C:/Users/am969/Documents/DFU_Proyect/SegmentationNetworks/Models/Unet++/output_assets_model/best_model_checkpoint_UnetPlusPlus.pth",  weights_only=True), model)
            print("Model loaded successfully!")
        except:
            print("Model not found. Training from scratch...")
    # if LOAD_MODEL:  # Colab
    #     try:
    #         load_checkpoint(torch.load("/content/DFU_Proyect/SegmentationNetworks/Models/Unet++/output_assets_model/best_model_checkpoint_UnetPlusPlus.pth",  weights_only=True), model)
    #         print("Model loaded successfully!")
    #     except:
    #         print("Model not found. Training from scratch...")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if SAVE_MODEL:
            ...
            if dc > best_dice_score:
                ...
                torch.save(checkpoint, "output_assets_model/best_model_checkpoint_UnetPlusPlus.pth")
                # torch.save(checkpoint, "my_checkpoint.pth.tar")
                with zipfile.ZipFile("output_assets_model/best_model_checkpoint_UnetPlusPlus.zip", 'w') as zipf:
                    zipf.write("output_assets_model/best_model_checkpoint_UnetPlusPlus.pth")
            else:
                cnt_patience += 1
        # Early stopping:
    ...
    if SAVE_MODEL:
        ...
        df.to_csv('output_assets_model/metrics_UnetPlusPlus.csv', index=False)     # Guardar el DataFrame en un archivo CSV
        ...
        pd.DataFrame(best_metrics, index=[0]).to_csv('output_assets_model/best_metrics_val(during_training)_UnetPlusPlus.csv', index=False)

        # Save parameters:
        parameters = {'Num Epochs': NUM_EPOCHS, 'Learning Rate': LEARNING_RATE, 'Batch Size': BATCH_SIZE, 'Image Height': IMAGE_HEIGHT, 'Image Width': IMAGE_WIDTH, 'Device': str(DEVICE), 'Num Workers': NUM_WORKERS, 'Pin Memory': PIN_MEMORY, 'Load Model': LOAD_MODEL, 'Save Images': SAVE_IMS, 'Train Image Dir': TRAIN_IMG_DIR, 'Val Image Dir': VAL_IMG_DIR, 'Elapsed Time[m]': round((end_time - start_time)/60, 4), 'Dropout Probability': DROPOUT_P, 'Patience (early_stop)': PATIENCE, 'Optimizer': 'Adam', 'Scheduler': 'ReduceLROnPlateau', 'Loss Function': 'Dice Loss', 'Scaler': 'GradScaler', 'Execution date': datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Model': 'UnetPlusPlus'}
        pd.DataFrame(parameters, index=[0]).to_csv('output_assets_model/parameters_UnetPlusPlus.csv', index=False)    # Guardar los parámetros en un archivo CSV
            # Guardar los parámetros como un archivo .json:
        with open('output_assets_model/parameters_UnetPlusPlus.json', 'w') as json_file:
