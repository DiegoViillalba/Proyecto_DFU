...
model1 = UnetPlusPlus(in_channels=3, out_channels=1).to(DEVICE)    ## ------------ Aquí al cambiar de modelo -------------.
...
pd.DataFrame(test_metrics, index=[0]).to_csv("output_assets_model/test_metrics_UnetPlusPlus.csv", index=False)
